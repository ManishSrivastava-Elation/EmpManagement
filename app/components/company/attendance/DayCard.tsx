    ...
    <View style={styles.punchCard}>
      ...
      {(punch.checkInImg || punch.checkOutImg) && (
        <View style={styles.imageRow}>
          {punch.checkInImg ? (
            <TouchableOpacity style={styles.imageBtn} onPress={() => { setSelectedImage(`${baseImageUrl}${punch.checkInImg!}`); setModalVisible(true); }}>
              <Ionicons name="camera-outline" size={15} color={color} />
              <Text style={[styles.imageText, { color }]}>Check‑in Photo</Text>
            </TouchableOpacity>
          ) : null}
          {punch.checkOutImg ? (
            <TouchableOpacity style={styles.imageBtn} onPress={() => { setSelectedImage(`${baseImageUrl}${punch.checkOutImg!}`); setModalVisible(true); }}>
              <Ionicons name="camera-outline" size={15} color={color} />
